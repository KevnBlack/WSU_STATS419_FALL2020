...
zipcode = '98682';  # CANADA allows strings for zipcodes, I removed, so only U.S.
...
```{r}
distances=conv_unit(distm(result.neighbors[,3:2],fun=distGeo),"m","mi"); # Distances in miles
result.neighbors$distance=distances[6,]; # Distances from 99163
result.neighbors$incircle=distances[6,]<=radius.miles; # T/F values if less than 10 miles
result.neighbors<-subset(result.neighbors,zipcode!=99163) # Remove 99163
```

#### TODO:  Choose a zipcode of your choice (not Pullman), and repeat everything above for that zipcode.
```{r}
distances=conv_unit(distm(result.neighbors[,3:2],fun=distGeo),"m","mi"); # Distances in miles
result.neighbors$distance=distances[41,]; # Distances from 98682
result.neighbors$incircle=distances[41,]<=radius.miles; # T/F values if less than 10 miles
result.neighbors<-subset(result.neighbors,zipcode!=98682) # Remove 98682
```
...
```{r}
library(wfindr)
find_word(allow="hello",type="anagram")
```
