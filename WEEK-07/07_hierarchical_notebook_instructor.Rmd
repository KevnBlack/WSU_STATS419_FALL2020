...
example.datasets.path = "D:/School/Fall 2020/STAT 419/git/WSU_STATS419_FALL2020/WEEK-07/pipe-format/";
...
# why would I do this?
protein.file2 = paste0(example.datasets.path,"protein.txt");
...
The red box around certain dendrograms represents a `best-guess` final answer.  In general, it appears there are two clusters on countries.  So although the individual `hclust` dendrograms suggested maybe more refined cluster.
...
for(method in methods)
  {
  time.start = Sys.time();  
      Xs.pvclust = pvclust ( protein.t, method.hclust=method);
  plot(Xs.pvclust);
      pvrect(Xs.pvclust);
  time.end = Sys.time();
  elapse = sprintf("%.3f", as.numeric(time.end) - as.numeric(time.start));
...
```{r}
X = removeColumnsFromDataFrame(protein.t,"Country");
rownames(X) = protein.t$Country;
...
for(method in methods)
  {
  time.start = Sys.time();  
      X.pvclust = pvclust ( cars, method.hclust=method);
  plot(X.pvclust);
      pvrect(X.pvclust);
  time.end = Sys.time();
  elapse = sprintf("%.3f", as.numeric(time.end) - as.numeric(time.start));
